...
public final class CameraManager {
     ...
     */
    public PlanarYUVLuminanceSource buildLuminanceSource(byte[] data, int width, int height, int parentWidth, int parentHeight) {
        int centerWidth = ViewfinderView.width * Math.min(width, height) / parentWidth;
        int centerHeight = ViewfinderView.height * Math.max(width, height) / parentHeight;
        if(width < height)
            return new PlanarYUVLuminanceSource(data, width, height, (width - centerWidth) / 2, (height - centerHeight) / 2, centerWidth, centerHeight, false); // Search QR in all image along, not only in Framing Rect as original code done
        else
            return new PlanarYUVLuminanceSource(data, width, height, (width - centerHeight) / 2, (height - centerWidth) / 2, centerHeight, centerWidth, false); // Search QR in all image along, not only in Framing Rect as original code done
    }
