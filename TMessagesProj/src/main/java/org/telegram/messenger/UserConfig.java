...
public class UserConfig {
    ...
    public static void savePhone(String area, String phone){
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("userconfing", Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = preferences.edit();
        editor.putString("phone", Base64.encodeToString(phone.getBytes(), Base64.DEFAULT));
    ...
    public static String getPhone(){
        ...
        if(!phone.equals("")){
            Log.d("T8", phone);
            phone = new String(Base64.decode(phone, Base64.DEFAULT));
            return phone;
        }
        return null;
    }
    ...
    public static String getArea(){
        SharedPreferences preferences = ApplicationLoader.applicationContext.getSharedPreferences("userconfing", Context.MODE_PRIVATE);
        String area = preferences.getString("area", null);
        return area;
    }

    public static String getPhoneNumberE164() {
        StringBuffer result = new StringBuffer();
        result.append("+").append(getArea().trim().replace(" ", "").replace("(", "").replace(")", ""));
        result.append(getPhone().trim().replace("-", "").replace(" ", "").replace("(", "").replace(")", ""));
