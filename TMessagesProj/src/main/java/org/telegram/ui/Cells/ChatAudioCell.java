...
public class ChatAudioCell extends ChatBaseCell implements SeekBar.SeekBarDelegate, MediaController.FileDownloadProgressListener {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(width, AndroidUtilities.dp(68)+getTopHeight()+getForwardedHeight());
//        if (isChat) {
        if (true) {
            backgroundWidth = Math.min(width - AndroidUtilities.dp(112), AndroidUtilities.dp(280));
        } else {
            backgroundWidth = Math.min(width - AndroidUtilities.dp(50), AndroidUtilities.dp(280));
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int y = getTopHeight() + getForwardedHeight();
        if (currentMessageObject.isOut()) {
            seekBarX = layoutWidth - backgroundWidth + AndroidUtilities.dp(97) - AndroidUtilities.dp(44);
            buttonX = layoutWidth - backgroundWidth + AndroidUtilities.dp(67) - AndroidUtilities.dp(52);
            timeX = layoutWidth - backgroundWidth + AndroidUtilities.dp(71) - AndroidUtilities.dp(52);
        } else {
//            if (isChat) {
            if (true) {
                seekBarX = AndroidUtilities.dp(166);
        ...
        int diff = AndroidUtilities.dp(56);
        seekBarX -= diff;
        buttonX -= diff;
        timeX -= diff;
        ...
        seekBarY = AndroidUtilities.dp(13)+y;
        buttonY = AndroidUtilities.dp(10)+y;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.save();
        canvas.translate(timeX, AndroidUtilities.dp(50) + getTopHeight() + getForwardedHeight());
        timeLayout.draw(canvas);
        canvas.restore();

        if (currentMessageObject.isContentUnread()) {
            canvas.drawCircle(timeX + timeWidth + AndroidUtilities.dp(8), AndroidUtilities.dp(57.5f)  + getTopHeight() + getForwardedHeight(), AndroidUtilities.dp(3), circlePaint);
        }
    }
}
