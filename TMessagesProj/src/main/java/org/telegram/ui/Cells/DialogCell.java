...
public class DialogCell extends BaseCell {
    ...
    private static Drawable unReadPostBadgeDrawable;
    ...
    private void init() {
        if (namePaint == null) {
            ...
            unReadPostBadgeDrawable = getResources().getDrawable(R.drawable.red_badge_bg);
        }
    }
    ...
    public void setDialog(TLRPC.TL_dialog dialog, int i, long unreadPostCount, boolean server) {
        currentDialogId = dialog.id;
        isDialogCell = true;
        index = i;
        isServerOnly = server;
    ...
    public void buildLayout() {
        ...
        if ( message != null && message.messageOwner != null && message.messageOwner.voteValue != 0) {
            timeString = message.messageOwner.voteValue + LocaleController.getString("Vote", R.string.Vote) + " " + timeString;
        }
        ...
        messageStringFinal = escapeAt(messageStringFinal.toString());
        messageStringFinal = Emoji.replaceEmoji(messageStringFinal, messagePaint.getFontMetricsInt(), AndroidUtilities.dp(17));
        try {
    ...
    public String escapeAt(String messageText) {
        ...
        if (p == null)
            p = Pattern.compile("(@.*?\\[\\d+\\])");
        matcher = p.matcher(messageText);
        while (matcher.find()) {
            String atText = matcher.group(1);
            String newAtText = atText.substring(0, atText.indexOf("["));
            int userId = Integer.valueOf(atText.substring(atText.indexOf("[") + 1, atText.indexOf("]")));
            TLRPC.User temp = MessagesController.getInstance().getUser(userId);
            if (temp != null) {
                String oldname = atText.substring(atText.indexOf("@") + 1, atText.indexOf("["));
                String newName = ContactsController.formatName(temp.first_name, temp.last_name);
                result = result.replace(atText, newAtText);
    ...
    public void checkCurrentDialogIndex(int dialogShowType) {
        TLRPC.TL_dialog dialog = null;
        if (isServerOnly) {
            switch (dialogShowType) {
                case DialogsAdapter.CHAT_DIALOG:
                    if (index < MessagesController.getInstance().chat_dialogsServerOnly.size()) {
                        dialog = MessagesController.getInstance().chat_dialogsServerOnly.get(index);
                    }
                    break;
                case DialogsAdapter.USER_DIALOG:
                    if (index < MessagesController.getInstance().user_dialogsServerOnly.size()) {
                        dialog = MessagesController.getInstance().user_dialogsServerOnly.get(index);
                    }
                    break;
            ...
            switch (dialogShowType) {
                case DialogsAdapter.CHAT_DIALOG:
                    if (index < MessagesController.getInstance().chat_dialogs.size()) {
                        dialog = MessagesController.getInstance().chat_dialogs.get(index);
                    }
                    break;
                case DialogsAdapter.USER_DIALOG:
                    if (index < MessagesController.getInstance().user_dialogs.size()) {
                        dialog = MessagesController.getInstance().user_dialogs.get(index);
                    }
                    break;
                case DialogsAdapter.ALL_DIALOG:
                    if (index < MessagesController.getInstance().dialogs.size()) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (unreadPostCount > 0) {
            Rect region = avatarImage.getDrawRegion();
            setDrawableBounds(unReadPostBadgeDrawable, region.right - AndroidUtilities.dp(5.5f), region.top, AndroidUtilities.dp(10), AndroidUtilities.dp(10));
            unReadPostBadgeDrawable.draw(canvas);
        }
    }
}
