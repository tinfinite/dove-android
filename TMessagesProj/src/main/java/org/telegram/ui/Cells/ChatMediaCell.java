...
public class ChatMediaCell extends ChatBaseCell implements MediaController.FileDownloadProgressListener {
    ...
    public void setMessageObject(MessageObject messageObject) {
        ...
        if (currentMessageObject != messageObject || isPhotoDataChanged(messageObject) || dataChanged) {
            ...
            if (messageObject.type == 9) {
                ...
                maxWidth = AndroidUtilities.getMinTabletSide() - AndroidUtilities.dp(162 + 86 + 24);
                } else {
                maxWidth = Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y) - AndroidUtilities.dp(154 + 86 + 24);
                }
            ...
            if (messageObject.type == 9) { //doc
                ...
                if (messageObject.messageOwner.media.title != null && messageObject.messageOwner.media.title.length() > 0) {
                    int maxWidth = (AndroidUtilities.isTablet() ? AndroidUtilities.getMinTabletSide() : Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y)) - AndroidUtilities.dp(86 + 86 + 56);
                ...
                if (AndroidUtilities.isTablet()) {
                    ...
                    photoWidth = (int) (Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y) * 0.6f);
                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(MeasureSpec.getSize(widthMeasureSpec), photoHeight + AndroidUtilities.dp(14) + getTopHeight() + additionHeight);
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (currentMessageObject.isOut()) {
            if (media) {
                x = layoutWidth - backgroundWidth - AndroidUtilities.dp(3) - AndroidUtilities.dp(52);
            } else {
                x = layoutWidth - backgroundWidth + AndroidUtilities.dp(6) - AndroidUtilities.dp(52);
            }
        } else {
            if (true) {
        ...
        y += getTopHeight();
        photoImage.setImageCoords(x, y+AndroidUtilities.dp(7), photoWidth, photoHeight);
        int size = AndroidUtilities.dp(48);
        buttonX = (int) (x + (photoWidth - size) / 2.0f);
        buttonY = (int)(AndroidUtilities.dp(7) + (photoHeight - size) / 2.0f) + y;

        radialProgress.setProgressRect(buttonX, buttonY, buttonX + AndroidUtilities.dp(48), buttonY + AndroidUtilities.dp(48));
        deleteProgressRect.set(buttonX + AndroidUtilities.dp(3), buttonY + AndroidUtilities.dp(3) + y, buttonX + AndroidUtilities.dp(45), buttonY + AndroidUtilities.dp(45));
    }
    ...
    protected void onAfterBackgroundDraw(Canvas canvas) {
        ...
        if (currentMessageObject.type == 9) {
            ...
            setDrawableBounds(menuDrawable, photoImage.getImageX() + backgroundWidth - AndroidUtilities.dp(44), AndroidUtilities.dp(10) + getTopHeight());
