...
public class ChatMessageCell extends ChatBaseCell {
    ...
    private boolean hasAddTimeHeight;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (currentMessageObject != null && currentMessageObject.textLayoutBlocks != null && !currentMessageObject.textLayoutBlocks.isEmpty() && currentMessageObject.messageText instanceof Spannable && !isPressed) {
            if (event.getAction() == MotionEvent.ACTION_DOWN || (linkPreviewPressed || pressedLink != null) && event.getAction() == MotionEvent.ACTION_UP) {
                ...
                if (x >= textX && y >= textY-10 && x <= textX + currentMessageObject.textWidth && y <= textY + currentMessageObject.textHeight+10) {
                    y -= textY;
                                        ...
                                        if (link[0] == pressedLink) {
                                            try {
                                                if (pressedLink instanceof URLSpanNoUnderline) {
                                                    ...
                                                    Utils.clickUrl(this, ((URLSpan) pressedLink).getURL());
...
//                                pressedLink.onClick(this);
                                ...
                                Utils.clickUrl(this, currentMessageObject.messageOwner.media.webpage.url);
...
//                if (isChat && !messageObject.isOut()) {
                if (!messageObject.isOut()) {
                    maxWidth = AndroidUtilities.getMinTabletSide() - AndroidUtilities.dp(142);
                    drawName = true;
                } else {
                    maxWidth = AndroidUtilities.getMinTabletSide() - AndroidUtilities.dp(142);
                    drawName = false;
...
//                if (isChat && !messageObject.isOut()) {
                if (!messageObject.isOut()) {
                    maxWidth = Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y) - AndroidUtilities.dp(142);
                    if(isChat)
                        drawName = true;
                    else
                        drawName = false;
            ...
            //Edit By PanJiafang. 该Cell的高度
//            totalHeight = messageObject.textHeight + AndroidUtilities.dp(32) + namesOffset;
            totalHeight = messageObject.textHeight + AndroidUtilities.dp(28) + getTopHeight() + getForwardedHeight();
            totalHeight = Math.max(totalHeight, AndroidUtilities.dp(48) + getTopHeight() + getForwardedHeight());
            ...
            if (messageObject.messageOwner.media instanceof TLRPC.TL_messageMediaWebPage && messageObject.messageOwner.media.webpage instanceof TLRPC.TL_webPage) {
                ...
                if (AndroidUtilities.isTablet()) {
                    if (currentMessageObject.messageOwner.to_id.chat_id != 0 && !currentMessageObject.isOut()) {
                        linkPreviewMaxWidth = AndroidUtilities.getMinTabletSide() - AndroidUtilities.dp(142);
                    } else {
                        linkPreviewMaxWidth = AndroidUtilities.getMinTabletSide() - AndroidUtilities.dp(142);
                    }
                } else {
                    if (currentMessageObject.messageOwner.to_id.chat_id != 0 && !currentMessageObject.isOut()) {
                        linkPreviewMaxWidth = Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y) - AndroidUtilities.dp(142);
                    } else {
                        linkPreviewMaxWidth = Math.min(AndroidUtilities.displaySize.x, AndroidUtilities.displaySize.y) - AndroidUtilities.dp(142);
                    }
                }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (currentMessageObject.isOut()) {
            ...
            textY = AndroidUtilities.dp(10) + getForwardedHeight() + getTopHeight();
        } else {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (currentMessageObject.isOut()) {
            textX = layoutWidth - backgroundWidth + AndroidUtilities.dp(10) - AndroidUtilities.dp(52);
            textY = AndroidUtilities.dp(10) + getForwardedHeight() + getTopHeight();
        } else {
//            textX = AndroidUtilities.dp(19) + (isChat ? AndroidUtilities.dp(52) : 0);
            textX = AndroidUtilities.dp(19) + AndroidUtilities.dp(52);
            textY = AndroidUtilities.dp(10) + getForwardedHeight() + getTopHeight();
        }
        ...
        if (firstVisibleBlockNum >= 0) {
            for (int a = firstVisibleBlockNum; a <= lastVisibleBlockNum; a++) {
                ...
                MessageObject.TextLayoutBlock block = currentMessageObject.textLayoutBlocks.get(a);
                canvas.save();
                float y = (totalHeight - getTopHeight() - getForwardedHeight() - currentMessageObject.textHeight - linkPreviewHeight)/2
                        + getTopHeight() + getForwardedHeight() + getTopOffset() + block.textYOffset;
                if(hasAddTimeHeight) {
                    if (hasLinkPreview)
                ...
                canvas.translate(textX - (int)Math.ceil(block.textXOffset), y);
        ...
        if (hasLinkPreview) {
            int startY = textY + currentMessageObject.textHeight + AndroidUtilities.dp(16);
