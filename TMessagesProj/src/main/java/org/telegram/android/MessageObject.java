...
public class MessageObject {
    ...
    private TLRPC.User user = UserConfig.getCurrentUser();
    private Pattern p;
                    ...
                    if (whoUser != null && fromUser != null) {
                        if (isOut()) {
                            if(whoUser.id == fromUser.id){
                                messageText = replaceWithLink(LocaleController.getString("JoinGroup", R.string.JoinGroup), "un1", whoUser);
                            } else
    ...
    private void generateLayout() {
        ...
        try {
            ...
            //Edit By PanJiafang. 此处可以修改行间距. 下方还有一次，需要修改
            textLayout = new StaticLayout(messageText, textPaint, maxWidth, Layout.Alignment.ALIGN_NORMAL, 1.2f, 0.0f, false);
        } catch (Exception e) {
            FileLog.e("tmessages", e);
            return;
        }
        ...
        for (int a = 0; a < blocksCount; a++) {
            ...
            if (blocksCount == 1) {
                ...
                try {
                    ...
                    block.textLayout = new StaticLayout(str, textPaint, maxWidth, Layout.Alignment.ALIGN_NORMAL, 1.2f, 0.0f, false);
...
//            messageText = messageText.toString().replace(atText, newAtText);
//            messageText = messageText.toString().replace(oldname, newName);
//        }
//        p = Pattern.compile("@.*?\\s");
        if(messageText.toString().contains("@"+user.username))
            result = true;
        return result;
    }
