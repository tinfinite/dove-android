...
import com.dlazaro66.qrcodereaderview.QRCodeReaderUtil;
...
public class PhotoViewer implements NotificationCenter.NotificationCenterDelegate, GestureDetector.OnGestureListener, GestureDetector.OnDoubleTapListener {
    ...
    private final static int gallery_menu_scan = 4;
    private final static int gallery_menu_crop = 5;
    ...
    public void setParentActivity(Activity activity) {
            ...
            public void onItemClick(int id) {
                if (id == -1) {
                    ...
                    if(centerImage != null){
                        ...
                        QRCodeReaderUtil.decodeFromFile(intArray, bitmap.getWidth(), bitmap.getHeight(), new QRCodeReaderUtil.OnQRCodeReadListener() {
                            @Override
                            public void onQRCodeRead(String text, PointF[] points) {
                                if(placeProvider != null) {
                                    placeProvider.onQRCodeRead(text, points);
                                    closePhoto(true, false);
                                }
                            }
        ...
        menuItem.addSubItem(gallery_menu_scan, LocaleController.getString("Scan", R.string.Scan), 0);
    ...
    private void setIndexToImage(ImageReceiver imageReceiver, int index) {
        ...
        /**
         * Edit By PanJiafang,  检测是否包含二维码
         */
        if(centerImage != null) {
            if (QRCodeReaderUtil.hasQRCode(centerImage.getBitmap()))
                menuItem.showSubItem(gallery_menu_scan);
            else
                menuItem.hideSubItem(gallery_menu_scan);
        }
